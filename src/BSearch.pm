...
sub search_file {
    ...
    $fh= BSearch::findpos($fh,0,$filesize,$coord,$field_no,$field_sep);
...
sub findpos { 
    my ($fh,$startpos,$endpos,$coord,$field_no,$field_sep) = @_;

    my $halfpos   = int(($endpos+$startpos)/2);

    my ($fh2,$halfstart,$halfend) = find_coord($fh,$halfpos,$field_no,$field_sep);
	...
	findpos($fh,$startpos,$halfpos,$coord,$field_no,$field_sep);
	...
	findpos($fh,$halfpos,$endpos,$coord,$field_no,$field_sep);
    }
}


sub find_coord {
    my ($fh,$halfpos,$field_no,$field_sep) = @_;
    ...
    return ($fh,$f[$field_no],$f[$field_no]);
